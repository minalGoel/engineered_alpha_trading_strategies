"""VWAP Overshoot Fade 003 — cursor_gpt54_vwap_overshoot_fade_003

Thesis: Same family as 001/002 but uses RSI(5) with wider thresholds (22/78)
and deeper VWAP deviation (0.7%/0.7%).  Requires higher relative volume (1.4).
VIX: 12-24.

Entry long: close <= VWAP*0.993, RSI(5) <= 22, rel_vol >= 1.4, ADX(14) <= 22,
    close > BB lower, abs(index_ret_15) <= 0.004, bullish bar.
Entry short: mirror with close >= VWAP*1.007, RSI(5) >= 78.
...
class Strategy(BaseStrategy):
    ...
    def tunable_params(self) -> list[TunableParam]:
        return [
            ...
            TunableParam("rsi_long_thresh", default=22.0, low=15.0, high=30.0),
            TunableParam("rsi_short_thresh", default=78.0, low=70.0, high=85.0),
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        ...
        rsi_l = params.get("rsi_long_thresh", 22.0)
        rsi_s = params.get("rsi_short_thresh", 78.0)
        ...
        rsi_fast = _compute_rsi_wilder(close, 5)  # RSI(5) for this variant
