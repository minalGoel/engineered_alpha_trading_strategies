"""Pharma Sector Pairs — cursor_opus46max_135

Thesis: Indian pharma stocks share common US generics market drivers.
When one stock diverges from the sector group, it reverts. Proxied by
stock vs index deviation z-score.
"""
import numpy as np
import polars as pl
...
class Strategy(BaseStrategy):
    name = "cursor_opus46max_135"
    is_long_only = False
    session_start = 570   # 09:30
    session_end = 915     # 15:15
    max_trades_per_day = 4
    assumptions = [
        "Pharma group data not available; using stock vs index as proxy",
    ]
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        ...
        # Index must be moving (sector active)
        idx_ret_from_open = np.abs((idx_close - idx_day_open) / safe_ido * 10000.0)
        idx_active = idx_ret_from_open > 15.0
        ...
        long_entry = long_signal & dev_ok & idx_active & vix_ok & time_ok
        short_entry = short_signal & dev_ok & idx_active & vix_ok & time_ok
