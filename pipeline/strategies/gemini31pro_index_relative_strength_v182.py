"""Index Relative Strength — cursor_gemini31pro_strategy_182

Thesis: Stocks showing resilience while index falls explode upwards when the
index stabilizes. 1min timeframe, nifty50. ROC lookback: 21.
Index threshold: -10 bps (long), stock > 2.7 bps.
VIX > 15. Volume: rel_vol > 1.2. Target: VWAP cross. Time stop: 120 bars.
"""
import numpy as np
import polars as pl
...
class Strategy(BaseStrategy):
    name = "gemini31pro_index_relative_strength_v182"
    is_long_only = False
    session_start = 570
    session_end = 915
    ...
    def tunable_params(self) -> list[TunableParam]:
        return [
            TunableParam("idx_thresh", default=10.0, low=5.0, high=40.0),
            TunableParam("stock_thresh", default=2.7, low=1.0, high=5.0),
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        idx_thresh = params.get("idx_thresh", 10.0)
        stock_thresh = params.get("stock_thresh", 2.7)
