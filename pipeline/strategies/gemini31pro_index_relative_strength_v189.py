"""Index Relative Strength — cursor_gemini31pro_strategy_189

Thesis: Stocks showing resilience while index falls explode upwards when the
index stabilizes. 5min timeframe, nifty50. ROC lookback: 14.
Index threshold: -20 bps, stock > 2.5 bps.
VIX: 12-22 range. Volume: rel_vol > 1.2. Target: VWAP cross. Time stop: 15 bars.
"""
import numpy as np
import polars as pl
...
class Strategy(BaseStrategy):
    name = "gemini31pro_index_relative_strength_v189"
    is_long_only = False
    session_start = 570
    session_end = 915
    ...
    def tunable_params(self) -> list[TunableParam]:
        return [
            TunableParam("idx_thresh", default=20.0, low=5.0, high=50.0),
            TunableParam("stock_thresh", default=2.5, low=0.5, high=5.0),
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        idx_thresh = params.get("idx_thresh", 20.0)
        stock_thresh = params.get("stock_thresh", 2.5)
        ...
        index_ret = _roc(index_close, 14)
        stock_ret = _roc(close, 14)
