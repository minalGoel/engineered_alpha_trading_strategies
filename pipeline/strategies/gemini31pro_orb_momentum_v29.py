...
"""ORB Momentum v29 — Gemini31Pro Strategy 029

Thesis: Opening range breakout with institutional order flow.
Uses HIGHEST(high,20)/LOWEST(low,20), VWAP confirmation,
volume > SMA(volume,30). Target 2.8 * ATR(30). Stop 1.5 * ATR(14).
...
class Strategy(BaseStrategy):
    name = "gemini31pro_orb_momentum_v29"
    is_long_only = False
    session_start = 570
    session_end = 915
    ...
    def tunable_params(self) -> list[TunableParam]:
        return [
            TunableParam("target_atr_mult", default=2.8, low=1.5, high=4.0),
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        target_mult = params.get("target_atr_mult", 2.8)
        ...
        # ── ORB: HIGHEST(high, 20) and LOWEST(low, 20) (shifted 1 bar to exclude current bar) ──
        # Shift by 1 so that we compare close[i] against the max/min of the PREVIOUS 20 bars.
        # Without the shift, close > orb_high is impossible because high[i] >= close[i].
        orb_high_raw = _rolling_max(high, 20)
        orb_low_raw = _rolling_min(low, 20)
