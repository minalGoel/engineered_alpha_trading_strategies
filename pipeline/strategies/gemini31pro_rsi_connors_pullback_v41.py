"""RSI Connors Pullback v41 — Gemini31Pro Strategy 041

Thesis: Short-term extreme oversold (RSI-2) within a longer-term uptrend
(close > SMA-200) indicates temporary liquidity exhaustion. Market makers
step in to bounce the price. Long when RSI(2) < 21 & close > SMA(200);
short when RSI(2) > 30 & close < SMA(200).
Signal exit: close > SMA(close,5) for longs, close < SMA(close,5) for shorts.
VIX < 25. Time stop 60 bars.
"""
import numpy as np
import polars as pl
...
class Strategy(BaseStrategy):
    name = "gemini31pro_rsi_connors_pullback_v41"
    is_long_only = False
    session_start = 570
    session_end = 915
    ...
    def tunable_params(self) -> list[TunableParam]:
        return [
            TunableParam("rsi_long_thresh", default=21.0, low=5.0, high=35.0),
            TunableParam("rsi_short_thresh", default=30.0, low=60.0, high=95.0),
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        rsi_long_thresh = params.get("rsi_long_thresh", 21.0)
        rsi_short_thresh = params.get("rsi_short_thresh", 30.0)
        ...
        # ── Entries ──
        long_entry = (close > sma_200) & (rsi_2 < rsi_long_thresh) & vix_ok & time_ok & vol_ok
        short_entry = (close < sma_200) & (rsi_2 > rsi_short_thresh) & vix_ok & time_ok & vol_ok
