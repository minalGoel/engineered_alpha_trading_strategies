"""VWAP Mean Reversion v14 — Gemini31Pro Strategy 014

Thesis: Retail panic sells push price far below VWAP; institutions fade.
Uses stddev(50) for z-score, SMA(volume,20) for rel vol.
Z-score thresh -2.2/+2.2, rel vol > 1.8. VIX > 15.
Additional volume filter: rel_volume > 1.2.
Time stop 30 bars.
"""
import numpy as np
import polars as pl
...
class Strategy(BaseStrategy):
    name = "gemini31pro_vwap_mean_reversion_v14"
    is_long_only = False
    session_start = 570
    session_end = 915
    ...
    def tunable_params(self) -> list[TunableParam]:
        return [
            TunableParam("zscore_thresh", default=2.2, low=1.2, high=3.0),
            TunableParam("rel_vol_thresh", default=1.8, low=1.0, high=2.5),
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        zs_thresh = params.get("zscore_thresh", 2.2)
        rel_vol_thresh = params.get("rel_vol_thresh", 1.8)
        ...
        vol_filter = rel_vol > 1.2  # additional volume filter

        # ── Entries ──
        long_entry = (
            (close < vwap) & (zscore < -zs_thresh) & (rel_vol > rel_vol_thresh)
            & vix_ok & time_ok & vol_filter
        )
        short_entry = (
            (close > vwap) & (zscore > zs_thresh) & (rel_vol > rel_vol_thresh)
            & vix_ok & time_ok & vol_filter
