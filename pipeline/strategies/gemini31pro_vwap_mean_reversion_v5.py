"""VWAP Mean Reversion v5 — Gemini31Pro Strategy 005
...
Uses stddev(14) for z-score, SMA(volume,30) for rel vol, vix > 15 filter.
Z-score thresh -2.7/+2.7, rel vol > 1.2. Time stop 15 bars.
"""
import numpy as np
import polars as pl
...
class Strategy(BaseStrategy):
    name = "gemini31pro_vwap_mean_reversion_v5"
    is_long_only = False
    session_start = 570
    session_end = 915
    ...
    def compute(self, df: pl.DataFrame, params: dict) -> StrategySignals:
        ...
        # ── Z-score: (close - vwap) / stddev(close, 14) ──
        dev = close - vwap
        std_14 = df["close"].rolling_std(14).to_numpy().astype(np.float64)
        std_14 = np.nan_to_num(std_14, nan=1e10)
        std_14 = np.clip(std_14, 1e-10, None)
        zscore = dev / std_14
